function Mtx_comp = Complement(Mtx_in, nbrvar)
    % STEP 1
    % if matrix is empty return one cube of 3
    if isempty(Mtx_in)
        Mtx_comp = ones(1, nbrvar)*3;
        
    % STEP 2 
    % if there is one 'don't care' cube return empty matrix
    ...
    elseif size(Mtx_in,1) == 1
      ...
      % create a sqaure matrix where index of row and col are linear
      % e.g instead of having indexOne(rowIndex,colIndex) we have number
      % so that index 1 is (1,1); 2 is (2,1) etc. Which is easier to manipulate
      % and be able to replace all 1 by 2. Those easy index are contained in indexOne
      ...
      % Create square matrix filled with 3
